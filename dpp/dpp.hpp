...
template <int ctrl>
__device__ __forceinline__ auto dpp(auto v) requires(sizeof(decltype(v)) == 4) {
...
template <typename T> __device__ __forceinline__ T dpp_wave_broadcast63(T val) {
...
template <typename T> __device__ __forceinline__ T dpp_wave_sum(T val) {
